...
export function ToolCallCard({ tool }: ToolCallCardProps): React.ReactElement {
  ...
  return (
    <div
      className={cn(
        'rounded-none border-[3px] border-black bg-white transition-colors',
    ...
    >
      ...
      >
        ...
        {isRunning ? (
          ...
          <Terminal className="h-3.5 w-3.5 shrink-0 text-black" />
        )}
        <span className="truncate font-mono text-xs text-black">{tool.name}</span>
        {summaryText && (
        ...
        <div className="border-t-[3px] border-black px-3 py-2">
          {Object.keys(tool.input).length > 0 && (
            <div className="mb-2">
              ...
              <pre className="mt-1 overflow-x-auto rounded-none border-[1px] border-black bg-[#f0f0f0] p-2 font-mono text-xs text-black">
          ...
          {tool.output !== undefined && (
            <div>
              ...
              <pre className="mt-1 max-h-80 overflow-auto rounded-none border-[1px] border-black bg-[#f0f0f0] p-2 font-mono text-xs text-black">
              ...
              {isLongOutput && !showAllOutput && (
                <button
                  ...
                  className="mt-1 font-sans text-xs font-semibold uppercase tracking-[0.05em] text-black hover:underline"
                >
