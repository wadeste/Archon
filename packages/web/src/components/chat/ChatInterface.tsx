    ...
    <div className="flex flex-1 flex-col overflow-hidden min-h-0">
      ...
      {(conversationsError || codebasesError) && (
        <div className="flex gap-2 px-4 py-1">
          {conversationsError && (
            <span className="font-mono text-xs text-[#ff0000]">Failed to load conversations</span>
          )}
          {codebasesError && (
            <span className="font-mono text-xs text-[#ff0000]">Failed to load projects</span>
          )}
        </div>
      )}
      <MessageList
