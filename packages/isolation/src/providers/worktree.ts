...
export class WorktreeProvider implements IIsolationProvider {
  ...
  private async createFromSameRepoPR(
    ...
    await execGitWithRetry(execFileAsync, 'git', ['-C', repoPath, 'fetch', remote, prBranch], {
      timeout: GIT_OPERATION_TIMEOUT_MS,
    });
  ...
  private async createFromForkPR(
    ...
    if (prSha) {
      ...
      await execGitWithRetry(
        execFileAsync,
        'git',
        ['-C', repoPath, 'fetch', remote, `pull/${prNumber}/head`],
        { timeout: GIT_OPERATION_TIMEOUT_MS }
      );
  ...
  private async createNewBranch(
    ...
    try {
      ...
      await execGitWithRetry(
        execFileAsync,
        'git',
        ['-C', repoPath, 'worktree', 'add', '--no-track', worktreePath, '-b', branchName, startPoint],
        { timeout: GIT_OPERATION_TIMEOUT_MS }
      );
    } catch (error) {
      const err = error as Error & { stderr?: string };
