...
interface RunLayersContext {
  ...
  workflowModel: string | undefined;
  workflowLevelOptions: WorkflowLevelOptions;
  /** Per-run model circuit breaker (F1 Archon-layer failover). Optional: loop_group
   *  body contexts don't carry one (v1) — guards use `?.` so bodies simply skip it. */
  modelCircuitBreaker?: RunModelCircuitBreaker;
  aiProfile?: ResolvedAiProfile;
  workflowPreset?: ModelAliasPreset;
  artifactsDir: string;
  /**
    ...
    const layerResults = await Promise.allSettled(
      layer.map(async (node): Promise<LayerNodeResult> => {
        try {
          ...
          const primaryModel =
            nodeOptions?.model ?? resolvedNodeModel ?? node.model ?? workflowModel;
          const nodeFallback =
          ...
          if (primaryModel && ctx.modelCircuitBreaker?.isOpen(provider, primaryModel)) {
            if (nodeFallback) {
          ...
          // `runAttempts` reads `activeOptions`/`activeModel` at call time so the
          // F1 fallback phase below reuses the same loop with the swapped model.
          const runAttempts = (): Promise<NodeExecutionResult> =>
            runNodeRetryLoop(
              node,
              platform,
              conversationId,
              ...
              () =>
                executeNodeInternal(
                  ...
                  configuredCommandFolder,
                  issueContext,
                  usedFallback && activeModel ? activeModel : resolvedNodeModel,
                  resolvedTier,
                  resolvedEffort,
                  stepNamePrefix,
                  iteration
          ...
          let output = await runAttempts();

          // Archon-layer model failover (F1). Engages on ANY error classification,
          // including FATAL — intentional: a different model often means a different
          // upstream provider/credential set, so even auth/credit errors on the
          // primary can be rescued by `on_failure_model`. Post-merge the fallback
          // gets its own full retry budget (a second runNodeRetryLoop pass) instead
          // of the primary's remaining attempts.
          if (output.state === 'failed' && !usedFallback && nodeFallback && activeModel === primaryModel) {
            // Record the primary-model failure at the moment the fallback engages —
            // otherwise nodes rescued by the fallback would never count against the
            // primary and its breaker could never open.
            if (primaryModel) {
              ctx.modelCircuitBreaker?.recordFailure(provider, primaryModel);
            }
            activeModel = nodeFallback;
            activeOptions = { ...nodeOptions, model: nodeFallback };
            usedFallback = true;
            ...
            output = await runAttempts();
          }

          if (output.state === 'failed') {
            // Attribute the terminal failure to the model that actually ran the last
            // attempt (the fallback when it engaged, the primary otherwise) — the
            // primary's failure was already recorded at the fallback-switch point.
            // retryCount is 0 post-merge: the attempt counter lives inside
            // runNodeRetryLoop now (per-attempt detail is T5 territory).
            if (activeModel) {
              ctx.modelCircuitBreaker?.recordFailure(provider, activeModel);
            }
            const detail = buildNodeFailureDetail({
              nodeId: node.id,
            ...
            output = attachFailureToOutput(output, detail) as NodeExecutionResult;
            await recordNodeFailure(deps, workflowRun, detail, node.command ?? node.id);
          }
...
export async function executeDagWorkflow(
  ...
  const runCtx: RunLayersContext = {
    modelCircuitBreaker: new RunModelCircuitBreaker(),
    deps,
    platform,
    conversationId,
    cwd,
